        ...
        if order['status'] == 'Pending' and new_status == 'In Progress':
            ...
            try:
                for item in order['cart']:
                    # Validate product availability and reserve if valid
                    valid, message = validate_and_reserve_product_availability(item['id'], item['quantity'])
                    if not valid:
                        # Rollback reservations for previously processed items
                    ...
                    # Reserve quantity for the current item
                    reserve_product_quantity(item['id'], item['quantity'])
                    adjusted_items.append(item)  # Track reserved item
                ...
                # Rollback reservations in case of any unexpected failure
                for adjusted_item in adjusted_items:
                    release_product_quantity(adjusted_item['id'], adjusted_item['quantity'])
...
def finalize_order(user_data, invoice_number):
    try:
        ...
        adjusted_items = []  # Track adjustments for rollback
        ...
        try:
            for item in order['cart']:
                ...
                adjust_product_quantity(item['id'], -item['quantity'])
                release_product_quantity(item['id'], item['quantity'])  # Finalize reserved quantities
            ...
            # Rollback changes
            for adjusted_item in adjusted_items:
                adjust_product_quantity(adjusted_item['id'], adjusted_item['quantity'])
                reserve_product_quantity(adjusted_item['id'], adjusted_item['quantity'])
            print(f"Error finalizing order: {str(e)}")  # Debug statement
            log_action(user_id, "finalize_order_error", {"error": str(e)})
